...
class SlideEmbedder(BaseSlideHandler):
    ...
    def extract_model_based_embeddings(
        ...
        """
        assert self.model is not None, "Model not found. Please provide an architecture name when initializing the SlideEmbedder."
        ...
        self.model_based_embedding_names = [f"{self.arch_name}_{i}" for i in range(embeddings_dim)]
    ...
    def extract_stain_based_embeddings(
        ...
        """
        log.info(f"Extracting embeddings from the patches of {slide_name}.")
        dataset = self.create_dataset(slide_name, slide_ext= slide_ext, coords=coords, no_transforms=True)
        dataloader = self.create_dataloader(dataset)
        if conv_matrix_name == "HED":
            stain_names = ["Hematoxylin", "Eosin", "DAB"]
        else:
            stain_names = ["Hematoxylin", "DAB"]
        self.stain_based_embedding_names = [f"{stain_name}_{feature}" for feature in STAIN_FEATURE_NAMES
    ...
    def extract_cell_based_embeddings(
        ...
        Args:
            ...
            show_progress: Whether to show the progress bar.
        """
        dataset = self.create_dataset(slide_name, slide_ext= slide_ext, coords=coords, coords_only=True)
        ...
        if "classification" not in cells_df.columns:
            raise ValueError("The 'classification' column is missing in the cells dataframe.")
        cell_classes = cells_df.classification.unique() if cell_classes is None else cell_classes
        self.cell_based_embedding_names = [f"{cell_class}_{feature}" for cell_class in cell_classes 
        ...
        for coords in tqdm(
            ...
            if self.num_workers > 1:
                cells_df_in_patches = pool.starmap(get_cells_in_patch, [(cells_df, coord, ref_patch_size, cell_classes) 
                                                                      for coord in coords])
                cells_features = np.array(pool.map(compute_cell_features, cells_df_in_patches))
            else:
                cells_df_in_patches = [get_cells_in_patch(cells_df, coord, ref_patch_size, cell_classes) for coord in coords]
                cells_features = np.array([compute_cell_features(cells_df_in_patch) for cells_df_in_patch 
                                           in cells_df_in_patches])
    ...
    def save_embeddings(
        ...
        slides_processed = set(self.model_based_embeddings.keys()).union(self.stain_based_embeddings.keys(),
                                                                         self.cell_based_embeddings.keys())
        for slide_name in slides_processed:
            if merge:
                embeddings_list = [("", torch.cat([emb_dict[slide_name] for emb_dict in
                                                   [self.model_based_embeddings, 
                                                   self.stain_based_embeddings,
                                                   self.cell_based_embeddings] if slide_name
                                                   in emb_dict.keys()], dim=1))]
            else:
                embeddings_list = [(emb_type, emb_dict[slide_name]) for emb_type, emb_dict in 
                                    zip(["_model_based", "_stain_based", "_cell_based"],
                                        [self.model_based_embeddings,
                                         self.stain_based_embeddings,
                                         self.cell_based_embeddings]) if slide_name in emb_dict.keys()]
    ...
    def save_embeddings_names(self,
                             save_dir: str,
                             format: str = "csv",
                             flush_memory: bool = False,
                             merge: bool = False,
        ...
        if merge:
            ...
            if os.path.exists(output_path):
                log.warning(f"File embeddings_names.{format} already exists in {save_dir}. Overwriting...")
            embeddings_names = self.model_based_embedding_names + self.stain_based_embedding_names \
                + self.cell_based_embedding_names
            df = pd.DataFrame(embeddings_names, columns=["feature_names"])
            df.to_csv(output_path, index=False)
        else:
            for emb_type, emb_names in zip(["model_based", "stain_based", "cell_based"], 
                                           [self.model_based_embedding_names, 
                                            self.stain_based_embedding_names,
                                            self.cell_based_embedding_names]):
                if len(emb_names) == 0:
                    continue
                output_path = os.path.join(save_dir, f"embeddings_names_{emb_type}.{format}")
                if os.path.exists(output_path):
                    log.warning(f"File embeddings_names_{emb_type}.{format} already exists in {save_dir}. Overwriting...")
