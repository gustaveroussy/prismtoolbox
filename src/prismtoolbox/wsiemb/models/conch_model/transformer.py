...
class AttentionalPooler(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        out = self.attn(
            self._repeat(q, N), x, x, need_weights=False, key_padding_mask=attn_mask
        )[0]
...
class MultimodalTransformer(Transformer):
    ...
    def forward(self, image_embs, text_embs):
        ...
        if self.mask_prob > 0.0 and self.training:
            ...
            attn_mask = attn_mask[None, :seq_len, :seq_len].repeat(
                batch_size, self.heads, 1, 1
            )
            # uniformly sample masking ratio from [0, mask_prob]
            p = random.random() * self.mask_prob
            # task_embs: [N, L, D]
            rand = torch.randn(
                text_embs.shape[:2], device=text_embs.device
            )  # [N, L] token pos.
            rand[:, 0] = -torch.finfo(
                rand.dtype
            ...
            indices = (
                rand.topk(num_mask, dim=-1).indices[:, None, :].repeat(1, seq_len, 1)
            )
